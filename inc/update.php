...
class PluginUpdateChecker_1_3 {
	 ...
	 * language
	 */
	public $M00001 = 'Check for updates';
	public $M00002 = 'The URL %s does not point to a valid plugin metadata file. ';
	public $M00003 = 'WP HTTP error: ';
	public $M00004 = 'HTTP response code is %s . (expected: 200) ';
	public $M00005 = 'wp_remote_get() returned an unexpected result.';
	public $M00006 = 'Can not to read the Version header for %s. The filename may be incorrect, or the file is not present in /wp-content/plugins.';
	public $M00007 = 'Skipping update check for %s - installed version unknown.';
	public $M00008 = 'This plugin is up to date.';
	public $M00009 = 'A new version of this plugin is available.';
	public $M00010 = 'Unknown update checker status "%s"';	
	...
	public function requestInfo($queryArgs = array()){
		...
		if ( !is_wp_error($result) && isset($result['response']['code']) && ($result['response']['code'] == 200) && !empty($result['body']) ){
			...
			$message = sprintf($this->M00002, $url);
			if ( is_wp_error($result) ) {
				$message .= $this->M00003 . $result->get_error_message();
			} else if ( isset($result['response']['code']) ) {
				$message .= sprintf($this->M00004, $result['response']['code']);
			} else {
				$message .= $this->M00005;
			}
	...
	public function checkForUpdates(){
		...
		if ( $installedVersion === null ) {
			if ( $this->debugMode ) {
				trigger_error(
					sprintf($this->M00007, $this->pluginFile),
					E_USER_WARNING
				);
			}
			return null;
	...
	public function addCheckForUpdatesLink($pluginMeta, $pluginFile, $pluginData = null, $status = null) {
		if ( $pluginFile == $this->pluginFile && current_user_can('update_plugins') ) {
			...
			$linkText = apply_filters('puc_manual_check_link-' . $this->slug, $this->M00001);
			if ( !empty($linkText) ) {
	...
	public function displayManualCheckResult() {
		if ( isset($_GET['puc_update_check_result'], $_GET['puc_slug']) && ($_GET['puc_slug'] == $this->slug) ) {
			...
			if ( $status == 'no_update' ) {
				...
				$message = sprintf($this->M00010, htmlentities($status));
			}
			printf(
